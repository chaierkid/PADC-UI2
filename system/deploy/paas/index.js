...
define(function(require) {
	...
	var paasConsoleURL = templateService.getXCloudConsoleUrl();
	...
	Model.prototype.modelLoad = function(event) {
		...
		$.ajax({
			...
			processData : false,
			type : 'GET',
			url : require.toUrl('$UI/system/deploy/paas/xPaasInfo.j?option=set&paasConsoleURL=' + paasConsoleURL),
			success : function(result) {
				var token = self.getContext().getRequestParameter("token");
				self.login(token);
				self.openPage({
					id : "deployInfo",
					url : "deployInfo.w",
	...
	Model.prototype.login = function(token) {
		var config = this.appEngine.getConfig();
		if (token && token !== "") {
			xpaas.login(token, function success(paasToken) {
				config.paasToken = paasToken;
			}, function fail(msg) {
				alert("登录失败，原因：" + msg);
				config.paasToken = "";
			});
		} else {
			// alert("未登录将无法使用CloudX5相关功能");
			config.paasToken = "";
		}
	};
	...
	Model.prototype.finish = function() {
		if (this.currentCompose.getInnerModel().finish(this)) {
			...
			this.appEngine.build(true, "$UI/system/deploy/paas/checkDeploy.j");
		} else {
		}
	};
